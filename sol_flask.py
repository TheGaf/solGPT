...
import markdownify  # Convert Markdown to HTML
...
# Google Drive API imports (optional RAG)
...
for var in ("SOL_GPT_PASSWORD", "BRAVE_API_KEY", "GROQ_API_KEY"):  # DRIVE vars optional
    ...
    for f in files:
        ...
        try:
            if mime == 'application/vnd.google-apps.document':
                req = drive_service.files().export_media(fileId=fid, mimeType='text/plain')
            else:
...
# --- SYSTEM PROMPT ---
SYSTEM_PROMPT = """You are Sol: a locally hosted AI assistant built specifically for Gaf..."""
...
try:
    ...
    try:
        with open(os.path.join(os.getcwd(), 'templates', 'sol.html')) as f:
            page_html = f.read()
    except Exception:
...
def sol_home():
    ...
    first = user_msg.lower().split()[0] if user_msg else ''
    casual = first in greetings and len(user_msg.split()) <= 2
    ...
    if uploaded_file and CLIP_AVAILABLE and not casual:
        try:
            ...
            emb_fn = OpenCLIPEmbeddingFunction()
            emb = emb_fn(data)
            res = text_collection.query(query_embeddings=[emb], n_results=1)
            image_context = res['documents'][0][0]
            image_source = res['metadatas'][0][0].get('source')
        except Exception as e:
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=10000, debug=False)  # prod
